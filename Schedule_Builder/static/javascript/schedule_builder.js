...
function defineAvailableClassList(class_list_data){
    class_list = class_list_data;
}

function addCourseToClassList(){
    let key = document.getElementById("selected_course").innerText;
    if (!(key in selected_class_list)) {
        selected_class_list[key] = class_list[key];
        let course = selected_class_list[key];
        ...
        let course_div = document.createElement("div");
        course_div.setAttribute("class", "course_div");
        course_div.id = "course_div-" + key;

        let li = document.createElement("li");

        let name = document.createElement("span");
        let nameText = document.createTextNode(course[0]["course_name"]);
        name.appendChild(nameText);
        name.id = course[0]["course_name"];
        ...
        course_div.onclick = function () {
            displayCourse(key);
            displayClassInformation(key)
        };
        displayCourse(key);
        hideEmptyClassContainer();
    }
}

function hideEmptyClassContainer(){
    let empty_class_container = document.getElementById("empty_class_container");
    empty_class_container.style.display = "none";
}
...
function createCourseDropDownButton(){
    ...
    for (let key in selected_class_list) {
        // addDropDownOption(key);
    }
...
function addDropDownOption(id){
    ...
    let course = selected_class_list[id];
    let a = document.createElement("a");
    let text = document.createTextNode(course[0]["course_name"]);
    a.appendChild(text);
    a.id = "dropdown_item-" + id;
    a.onclick = function () {displayCourse(id);};
    dropdown.appendChild(a);
}
...
function displayCourse(id){
    ...
    let displayed_table = document.getElementById("table-" + id);
    if (displayed_table){                             // If the table exists, then change style to show table
        displayed_table.style.display = "table";
    }
...
function filter_rows() {
    ...
    for (let key in selected_class_list){                        // Go through all the tables
        let course = selected_class_list[key];
...
function displayVisualSchedule(){
